...
@author: Kaiqi Yuan
@software: PyCharm
@file: attentive_description_deepwalk.py
@time: 2019/2/27 16:27
@description: 以hierarchy_deepwalks为base，对hierarchy_description作attention,
              以hierarchy_description为base，对hierarchy_deepwalks作attention,
"""
...
from util.single_attention_layer import max_pooling_attention, mutual_attention
import torch
...
class AttentiveDescriptionDeepwalkModel(BaseModel):
    ...
    def train(self):
        ...
        # attention variable
        self.description_weight = Variable(torch.randn(self.hierarchy_gru_description.get_out_dims(), 10)).double().to(self.device)
        self.deepwalk_weight = Variable(torch.randn(self.hierarchy_gru_deepwalk.get_out_dims(), 10)).double().to(self.device)
        self.description_V = Variable(torch.randn(10, 1)).double().to(self.device)
        self.deepwalk_V = Variable(torch.randn(10, 1)).double().to(self.device)
        ...
        for epoch in range(self.NUM_EPOCHS):
            ...
            for i, (phy_features, target_seqs, labels) in enumerate(self.train_loader):
                ...
                # phamrmacology_attention
                att_hierarchy_descriptions,  __ = mutual_attention(hierarchy_description, drug_deepwalk, self.description_V, self.description_weight, self.device)
                att_hierarchy_deepwalks, _ = mutual_attention(drug_deepwalk, hierarchy_description, self.deepwalk_V, self.deepwalk_weight, self.device)
                ...
                #       att_hierarchy_deepwalks.size())
                # inputs = torch.cat((phy_features, target_seqs, att_hierarchy_descriptions, att_hierarchy_deepwalks),
                #                    1).reshape((this_batch_size, 1, -1, 1))
                inputs = torch.cat((phy_features, target_seqs,
                                    ...
                                    att_hierarchy_descriptions, att_hierarchy_deepwalks), 1).reshape(
    ...
    def test(self, test_data_file):
        ...
        with torch.no_grad():
            ...
            for i, (phy_features, target_seqs, labels) in enumerate(test_loader):
                ...
                # pharmacology_attention
                att_hierarchy_descriptions, __ = mutual_attention(hierarchy_description, drug_deepwalk,
                                                                  self.description_V, self.description_weight,
                                                                  self.device)
                att_hierarchy_deepwalks, _ = mutual_attention(drug_deepwalk, hierarchy_description, self.deepwalk_V,
                                                              self.deepwalk_weight, self.device)
                ...
                #     (this_batch_size, 1, -1, 1))
                # inputs = torch.cat((phy_features, target_seqs, att_hierarchy_descriptions, att_hierarchy_deepwalks), 1).reshape(
                #     (this_batch_size, 1, -1, 1))
                inputs = torch.cat((phy_features, target_seqs,
                                    hierarchy_description.reshape((this_batch_size, -1)),
                                    drug_deepwalk.reshape((this_batch_size, -1)),
...
if __name__ == '__main__':
    model = AttentiveDescriptionDeepwalkModel(PharmacologyDescriptionDeepwalkDataset, "../Data/DTI/e_train.pickle", "e_attentive_description_deepwalk_model.ckpt")
    model.train()
    model.classifier.eval()
    model.test("../Data/DTI/e_test.pickle")

    # model = AttentiveDescriptionDeepwalkModel(PharmacologyDescriptionDeepwalkDataset, "../Data/DTI/ic_train.pickle", "ic_attentive_description_deepwalk_model.ckpt")
    # model.train()
    # model.classifier.eval()
    # model.test("../Data/DTI/ic_test.pickle")
    #
    # model = AttentiveDescriptionDeepwalkModel(PharmacologyDescriptionDeepwalkDataset, "../Data/DTI/gpcr_train.pickle", "gpcr_attentive_description_deepwalk_model.ckpt")
    # model.train()
    # model.classifier.eval()
