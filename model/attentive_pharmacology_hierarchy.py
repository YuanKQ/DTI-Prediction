...
@author: Kaiqi Yuan
@software: PyCharm
@file: attentive_pharmacology_hierarchy.py
@time: 2019/2/27 16:09
@description: 以药物药理特性作为base,
              分别对药物描述信息，药物network_embedding信息作attention
"""

import sys
sys.path.extend(['/home/kqyuan/DDISuccess', '/home/kqyuan/DDISuccess/util',
                 '/home/lei/DDISuccess', '/home/lei/DDISuccess/util'])
from util.single_attention_layer import max_pooling_attention, pharmacology_attention
import torch
...
class AttentivePharmacologyHierarchyModel(BaseModel):
    ...
    def train(self):
        ...
        # attention variable
        self.description_weight = Variable(torch.randn(self.hierarchy_gru_description.get_out_dims(), self.pharmacology_cnn.get_out_dims())).double().to(self.device)  # 2 for bidirection
        self.deepwalk_weight = Variable(torch.randn(self.hierarchy_gru_deepwalk.get_out_dims(), self.pharmacology_cnn.get_out_dims())).double().to(self.device)  # 2 for bidirection
        ...
        for epoch in range(self.NUM_EPOCHS):
            ...
            for i, (phy_features, target_seqs, labels) in enumerate(self.train_loader):
                ...
                att_hierarchy_descriptions,  __ = pharmacology_attention(hierarchy_description, phy_features, self.description_weight, self.device)
                att_hierarchy_deepwalks, _ = pharmacology_attention(drug_deepwalk, phy_features, self.deepwalk_weight, self.device)
    ...
    def test(self, test_data_file):
        ...
        with torch.no_grad():
            ...
            for i, (phy_features, target_seqs, labels) in enumerate(test_loader):
                ...
                att_hierarchy_descriptions, __ = pharmacology_attention(hierarchy_description, phy_features, self.description_weight,
                                                                        self.device)
                att_hierarchy_deepwalks, _ = pharmacology_attention(drug_deepwalk, phy_features, self.deepwalk_weight, self.device)
...
if __name__ == '__main__':
    # model = AttentivePharmacologyHierarchyModel(PharmacologyDescriptionDeepwalkDataset, "../Data/DTI/e_train.pickle", "e_attentive_pooling_network_model.ckpt")
    # model.train()
    # model.classifier.eval()
    # model.test("../Data/DTI/e_test.pickle")

    # model = AttentivePharmacologyHierarchyModel(PharmacologyDescriptionDeepwalkDataset, "../Data/DTI/ic_train.pickle", "ic_attentive_pooling_network_model.ckpt")
    # model.train()
    # model.classifier.eval()
    # model.test("../Data/DTI/ic_test.pickle")
    #
    model = AttentivePharmacologyHierarchyModel(PharmacologyDescriptionDeepwalkDataset, "../Data/DTI/gpcr_train.pickle", "gpcr_attentive_pooling_network_model.ckpt")
    model.train()
    model.classifier.eval()
